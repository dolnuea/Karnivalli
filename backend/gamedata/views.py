...
class GameDataRetriveList(RetrieveUpdateDestroyAPIView):
    ...
    def get(self, request, format='json'):
        ...
        request.data["owner"] = User.objects.filter(username=request.data["owner"])[0].id
        gamedataList = GameData.objects.filter(Q(owner=request.data["owner"]) | Q(opponent=request.data["owner"]))
        output_data = []
        for gameData in gamedataList:
            output_data.append({"owner":gameData.owner.username, "game_session_id":gameData.game_session_id, "game":gameData.game.game_name, "opponent": gameData.opponent.username})
        print(output_data)
...
class UserGameScoreCreate(ListCreateAPIView):
    ...
    def post(self, request, format='json'):
        ...
        if not gameScoreData: 
            ...
            gameScoreData.update(score = request.data["score"])
